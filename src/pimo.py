...
class Main():
    ...
    def execute_command(self, command:str):
        ...
        to_log = "-"
        if len(output_lines):
            to_log = output_lines[0].strip()
            if len(output_lines) > 1:
                to_log += f"\n{self.logger.start}    ↳ ".join(output_lines[1:])
    ...
    def start(self):
        ...
        if not self.direct:
            self.logger.log(f"Here's the output files who will be made after compilation : {self.output}, {self.assembly_output}", "info")
        else:
            self.logger.log(f"Here's the output file who will be made after compilation : {self.output}", "info")
    ...
    def end(self):
        if self.timer:
            ...
            self.logger.log(f"Compilation time : {self.end_time - self.start_time}s", "info")
